...
class ML_Classifier(BaseEstimator, ClassifierMixin): 
    def __init__(self, model_name='CSP', **kwargs):
        ...
        elif self.model_name == 'RSF-SBLEST':
            Process = RSF(self.rsf_dim, self.rsf_method)
            Basemodel = SBLEST(K=2, tau=1, device=self.device)
            self.clf = make_pipeline(Process, Basemodel)
        else:
