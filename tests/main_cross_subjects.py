...
def get_neighbors(subjects, target_index, num_neighbors=5):
    # 如果num_neighbors过大，返回全部subjects
    if num_neighbors * 2 + 1 >= len(subjects):
        return subjects
    neighbors = [(target_index + i) % len(subjects) for i in range(-num_neighbors, num_neighbors + 1)]
    return [subjects[i] for i in neighbors]
...
if __name__ == '__main__':
    ...
    # 定义结果保存目录
    folder = f'./results/cross_subjects/{dataset_name}'
    ...
    for sub in subjects:
        ...
        target_index = subjects.index(sub)
        neighbors = get_neighbors(subjects, target_index)
        print(f"target sub={sub}, all sub: {neighbors}")

        #将neighbors中每个列表元素-1，因为python的索引从0开始
        neighbors_indices = [i - 1 for i in neighbors]
        datas_new = [datas[i] for i in neighbors_indices]
        labels_new = [labels[i] for i in neighbors_indices]
